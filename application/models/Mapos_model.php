...
class Mapos_model extends CI_Model
{
    ...
    public function calendario($start, $end, $status = null)
    {
        $this->db->select(
            ...
            COALESCE((SELECT SUM(lancamentos.valor - lancamentos.valor_ajuste) FROM lancamentos WHERE lancamentos.tipo = "receita"), 0) totalReceitas,
            COALESCE((SELECT SUM(lancamentos.valor - lancamentos.valor_ajuste) FROM lancamentos WHERE lancamentos.tipo = "despesa"), 0) totalDespesas'
        );
       ...
       // $this->db->join('servicos_os', 'servicos_os.os_id = os.idOs', 'left');
         $this->db->where('lancamentos.data_pagamento >=', $start);
         $this->db->where('lancamentos.data_vencimento <=', $end);
         $this->db->group_by('lancamentos.idLancamentos');

        if (!empty($status)) {
            $this->db->where('lancamentos.tipo', $status);
        }
