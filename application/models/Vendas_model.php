...
class Vendas_model extends CI_Model
{
    ...
    {
        $this->db->select('itens_de_vendas.*, produtos.*, medidas.*, medidas_sistema.*');
    ...
    public function autoCompleteProduto($q)
    {
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row) {
                $estoque = intDiv($row['estoque'], $row['multiplicador']);
                $resto = $row['estoque'] % $row['multiplicador'];
                $row_set[] = ['label'=>$row['descricao'].' | Preco: R$ '.$row['precoVenda'].' | Estoque: '.$estoque.'-'.$row['sigla'],'estoque'=>$estoque,'id'=>$row['idProdutos'],'preco'=>$row['precoVenda'],'unidade'=>$row['unidade'],'categoriaId'=>$row['categoriaId'],'marca'=>$row['marca'],'produto'=>$row['complemento'],'caracteristicas'=>$row['caracteristicas'],'resto'=>$resto]; 
            }
