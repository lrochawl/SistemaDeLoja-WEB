...
class Mapos extends MY_Controller
{
    ...
    public function calendario()
    {
        ...
        $events = array_map(function ($lancamentos) {
            ...
            return [
                'title' => "Lancamento: {$lancamentos->idLancamentos}, Cliente: {$lancamentos->cliente_fornecedor}",
                'start' => $lancamentos->data_vencimento,
                'end' => $lancamentos->data_vencimento,
                'color' => $cor,
                'extendedProps' => [
                    'id' => $lancamentos->idLancamentos,
                    ...
                    'observation' => '<b>Observação:</b> ' . $lancamentos->observacao,
                    ...
                    'valorDesconto' => '<b>Valor Desconto:</b> R$ ' . number_format($lancamentos->valor_desconto, 2, ',', '.'),
                    'valorPago' => '<b>Valor Pago:</b> R$ ' . number_format($lancamentos->valor_pago, 2, ',', '.'),
                    'editar' => $this->financeiro_model->isEditable($lancamentos->idLancamentos),
                ]
            ];
        }, $allLancamentos);
