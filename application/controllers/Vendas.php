...
class Vendas extends MY_Controller
{
    ...
    public function consultaMedidas( )
    {
        $data =$this->ConfigProduto_model->getMedida('medidas', '*', '', $this->uri->segment(3));
    ...
    public function adicionarProduto()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $medida = explode(" ",$this->input->post('medida'));
            $unidade = explode(" ",$this->input->post('unidade'));
            $nomeMedida = $unidade[1]; 
            $quantidade = $this->input->post('quantidade'); 
            $fracionador = intval($medida[0]);
            $medida = intval($medida[1]); 
            
           if(intval($unidade[0]) == 1){
                $quantidade = $quantidade / $fracionador; 
            }elseif(intval($unidade[0]) == 2){
                $quantidade = $quantidade; 
            }elseif(intval($unidade[0]) == 3){
                $quantidade = $quantidade * $medida; 
            }
            
            $preco = $this->input->post('preco');
            $precoMedida = ($preco / $medida);            
            $subtotal = 0;
            $produto = $this->input->post('idProduto');
            $subtotal = ($precoMedida * $quantidade);
            $quantidadeEstoque = $this->input->post('quantidade') / $fracionador;

         
          //  $total_preco = ($preco / $medida) * ($quantidade * $unidade);
            $data = [
                'unidadeVenda'=> $nomeMedida,
                'quantidade' => $this->input->post('quantidade'),
                'subTotal' => $subtotal,
                'produtos_id' => $produto,
                'preco' => $precoMedida,
            ...
            if ($this->vendas_model->add('itens_de_vendas', $data) == true) {
                ...
                if ($this->data['configuration']['control_estoque']) {
                    $this->produtos_model->updateEstoque($produto, $quantidadeEstoque, '-');
                }

                $this->db->set('ajustaValor', 0.00);
                $this->db->set('valor_ajuste', 0.00);
    ...
    public function faturar()
    {
        ...
        if ($this->form_validation->run('receita') == false) {
            ...
            $data = [
                ...
                'valor_ajuste_tipo' => $vendas->valor_ajuste_tipo,
                'clientes_id' => $this->input->post('clientes_id'),
