                                ...
                                <div class="span11">
                                    <form id="formProdutos" action="<?php echo base_url(); ?>index.php/vendas/adicionarProduto" method="post">
                                        ...
                                        <div class="span2">
                                            ...
                                            <input type="text" placeholder="Preço (R$)" id="preco" name="preco" class="span5 money" readonly />
                                ...
                                <table class="table table-bordered" id="tblProdutos">
                                    ...
                                    <tbody>
                                        ...
                                        foreach ($produtos as $p) {
                                           $casasDecimais = ($p->unidadeVenda == $p->sigla)?0:2;
                                            $total +=  $p->subTotal;
                                          
                                                $quatitade = $p->quantidade ;
                                                $preco = number_format($p->preco, 2, ',', '.').'('.$p->siglaMedidaSistema.')'; 
                                           
                                            echo '<tr>';
                                            echo '<td>' . $p->codDeBarra . '</td>';
                                            echo '<td>' . $p->descricao . '</td>';
                                            echo '<td><div align="center">' . number_format($p->quantidade, $casasDecimais, ',', '.')."-".$p->unidadeVenda . '</td>';
                                            echo '<td><div align="center">R$: ' . $preco  . '</td>';
                                            echo '<td><div align="center"><a href="" idAcao="' . $p->idItens . '" prodAcao="' . $p->idProdutos . '" quantAcao="' . $quantidade. '" title="Excluir Produto" class="btn-nwe4"><i class="bx bx-trash-alt"></i></a></td>';
                                    ...
                                    <tfoot>
                                        ...
                                        ?>
                                            <tr>
                                                <td colspan="4" style="text-align: right"><strong><?php echo ucwords(strtolower($result->ajustaValorTipo)) ?>:</strong></td>
                                                <td>
                                                    <div align="center"><strong>R$: <?php echo number_format($result->ajustaValor, 2, '.', ','); ?> </strong></div>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td colspan="4" style="text-align: right"><strong>Total Com <?php echo ucwords(strtolower($result->ajustaValorTipo)) ?>:</strong></td>
                                                <td>
                                                    <div align="center"><strong>R$: <?php echo number_format($result->valor_ajuste, 2, ',', '.'); ?></strong></div><input type="hidden" id="total-desconto" value="<?php echo number_format($result->valor_ajuste, 2); ?>">
                                                </td>
                                            </tr>
                                        <?php
                                        } ?>
...
console.log($("#total-venda").val() == null || $("#total-venda").val() == '');
    ...
    $(document).ready(function() {
        ...
        $("#produto").autocomplete({
            ...
            select: function(event, ui) {
                ...
                dados.append('id', ui.item.id);
                dados.append('estoque', ui.item.estoque);
                dados.append('preco', ui.item.preco);
                $.ajax({
                    url: "<?php echo base_url(); ?>index.php/vendas/consultaMedidas",
                    method: 'POST',
                    data: dados,
                    processData: false,
                    contentType: false,
                    success: function(data) {
                        var data = JSON.parse(data);
                            ...
                            function calculaPrecoMedida(e){
                                if(e['sigla'] == ui.item.unidade){
                                   
                                    $('#medida').val(e['fracionadorMedidaSistema']+' '+e['multiplicador']);
                                    $("#unidade option").remove();
                                    

                                    if(e['siglaMedidaSistema'] != 'UN'){
                                        $("#unidade").append("<option value='0'>Selecione</option>");
                                        $("#unidade").append("<option value='1 "+e['siglaFracaoSistema']+"' >"+e['siglaFracaoSistema']+"</option>");
                                        $("#unidade").append("<option value='2 "+e['siglaMedidaSistema']+"' >"+e['siglaMedidaSistema']+"</option>");
                                        if(e['sigla'] != e['siglaMedidaSistema']){
                                            $("#unidade").append("<option value='3 "+e['sigla']+"' >"+e['sigla']+"</option>");
                                        }
                                    ...
                                    if(e['siglaMedidaSistema'] == 'UN'){
                                        if(e['sigla'] != e['siglaMedidaSistema']){
                                            ...
                                            $("#unidade").append("<option value='3 "+e['siglaFracaoSistema']+"' >"+e['siglaFracaoSistema']+"</option>");
                                        } 
                            ...
                            data.forEach(calculaPrecoMedida);
